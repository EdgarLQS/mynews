...
- `ApplicationRuntime` 在 M2 中已 Implemented；`ArtifactCommitter` 在 M3 中已 Implemented；
  `evaluate` 在 M4 已完成离线 Implemented；`ops` 在 M5 已完成离线 Implemented；M6
  共享结构化 Codex 进程 Adapter 已完成离线 Implemented；`editorial review` 已完成离线
  Implemented，但真实输入和生产 Codex 建议仍不得写成 Verified。
