...
> 当前状态：v1.6 已按 Implemented 归档，v1.7 分时情报分析与人工反馈闭环的 P0–P4 已按离线门禁实现；M2–M7 的离线能力已按 `Implemented` 交付。真实 Codex 双档、latest-only 补跑、故障恢复、质量和编辑复盘验收仍须独立取证。网络受限时保持 `BLOCKED`，不得用任务文档或 mock 代替 Verified。外部插件是受信任本地 Python 代码，显式清单不是进程级沙箱。
...
uv run mynews ops recovery-check --source-root . --target /tmp/mynews-recovery --out-dir /tmp/mynews-ops
...
uv run mynews editorial review --week 2026-W32 \
  --out-dir output/editorial/reviews --no-codex
```
...
`mynews editorial review` 只读历史 Candidate、Digest、publication ledger 和 weekly feedback，
输出 `EditorialReview 1.0` 的周 JSON/Markdown 及 latest 双格式；缺少四个完整 ISO 周、十条发布记录
或对应反馈时返回 `partial` 和缺失原因。`--no-codex` 仍生成确定性报告，Codex 只提供可丢弃的建议，
不能增加事实、事件引用或 URL；M7 当前为离线 `Implemented`，不代表真实编辑质量 `Verified`。
