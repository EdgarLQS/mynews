...
> 当前状态：v1.4 外部来源插件已按 Implemented 归档；v1.5 扩展来源与安全交接处于 Proposed，计划以独立插件包增加来源并保持默认采集不变。v1.3 Digest 的真实 Codex 记录仍为 `BLOCKED`，真实 launchd 按验收边界未加载。外部插件是受信任本地 Python 代码，显式清单不是进程级沙箱。
