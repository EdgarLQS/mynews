...
8. For v1.7, verify that `news-task.md` keeps formal items limited to verified Digest main items, implements Asia/Shanghai 09:00/18:00 latest-only and report-before-state rules, and never updates publication feedback or produces platform assets automatically. Verify publication/feedback CLI candidate matching, complete fields, idempotency, explicit replacement, privacy, no network/Store/Codex side effects, and atomic recovery; P5 real dual-slot scheduling remains SKIPPED unless separately authorized.
9. Treat a task document as Implemented evidence only. Do not call the scheduled workflow Verified until an authorized real Codex task completes both slots, latest-only catch-up, and failure recovery; no registered task is SKIPPED/BLOCKED rather than PASS.
10. Distinguish failures introduced by the scoped changes from pre-existing or unconfirmed failures, and return the exact conclusion vocabulary and self-contained report structure defined by the acceptance rules.
11. For an “实施验收” or explicit “验收并修复” request, finish and report the read-only pass first, then fix as a separate development step and rerun affected gates plus full acceptance.
