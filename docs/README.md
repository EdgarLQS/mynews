...
| v1.7 收口至 v2.0 统一开发总计划 | Current / In Progress | M0 已完成；P5 真实双档仍为 BLOCKED，M2–M7 已按用户限定范围完成离线实施，未将 P5 或后续真实能力写成 Verified |
...
| 编辑周复盘 | Implemented | `editorial review` 只读 Candidate、Digest、发布台账和周反馈，输出 EditorialReview 1.0；数据不足 partial，Codex 建议可选且不提供事实 |
| 运行可靠性 | Implemented | `ops` 诊断、非破坏性保留计划和白名单隔离恢复检查；真实恢复演练仍未 Verified |
...
| 文档 | 状态 | 实现状态 |
| --- | --- | --- |
| [统一开发总计划](planning/v1.7-v2.0-master-plan.md) | Current | In Progress；M2–M7 已在限定范围下 Implemented，M1/P5 与后续真实能力仍不得写成 Verified |
...
| 文档 | 状态 | 实现状态 |
| --- | --- | --- |
| [架构方向审查](architecture/architecture-direction-review-2026-08-20.md) | Draft | Proposed；整理当前设计与候选方向，M2/M3/M4 已在统一总计划中锁定，系统架构仍是当前结构真相 |
