...
| VER-02 | 核验 | 可配置 Codex Verifier 与单次候选预算 | Implemented | Fake/预算/批次测试；生产 Codex G6-V 待真实环境 |
| VER-03 | 核验 | URL、域名、重定向、摘录、日期和哈希二次校验 | Implemented | 伪造来源、跨域重定向和严格契约测试 |
| VER-04 | 核验 | discovery 候选进入生产 Codex，模型不得扩大白名单 | Implemented | 离线 seam 已通过；真实 `codex_primary_evidence` G6-V 待执行 |
...
| OPS-03 | 运维 | 隔离七天回溯、validate 和 report | Implemented | 同一固定窗口连续两次真实运行 G7 待执行 |
