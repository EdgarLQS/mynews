...
| ARCH-02 | 架构 | 统一 `ArtifactCommitter` 文件提交 module | Implemented | M3 `commit(writes)` 统一暂存、fsync、替换、回滚和临时清理；保持 latest、历史和报告先于状态语义兼容；当前全量测试 339 项 |
...
| DATA-08 | 数据 | QualityEvaluation、Operations、EditorialReview 独立 1.0 Schema | In progress | M4 QualityEvaluation 与 M5 Operations 已完成离线 1.0；EditorialReview 仍待 M7，不修改 RunReport、Candidate、Digest 或 automation state |
