...
M2–M7 已按离线限定 Implemented；用户于 2026-08-26 明确允许在 P5 保持 BLOCKED 的前提下继续离线实施，
该豁免不改变真实能力状态。
...
| ARCH-02 | 架构 | 统一 `ArtifactCommitter` 文件提交 module | Implemented | M3 `commit(writes)` 统一暂存、fsync、替换、回滚和临时清理；保持 latest、历史和报告先于状态语义兼容；当前全量测试 347 项 |
| ARCH-03 | 架构 | 共享结构化 Codex 进程 Adapter | Implemented | M6 `CodexProcessAdapter` 统一临时目录、Schema、只读命令、超时、返回码和输出读取；不合并领域 prompt、响应模型或复核；生产 Codex 仍需真实验收 |
...
| CLI-15 | CLI | `mynews ops` 诊断、保留计划与隔离恢复检查 | Implemented | M5 离线 Operations 1.0、非破坏性候选、白名单恢复和原子报告；真实恢复演练仍未 Verified |
| CLI-16 | CLI | `mynews editorial review` 周复盘与只读建议 | Implemented | M7 离线 EditorialReview 1.0；四周/十条发布/反馈不足时 partial，Codex 建议可选且可回退 |
...
| DATA-08 | 数据 | QualityEvaluation、Operations、EditorialReview 独立 1.0 Schema | Implemented | M4–M7 三个独立 1.0 Schema 均完成离线实现；不修改 RunReport、Candidate、Digest 或 automation state |
...
| EDITORIAL-03 | 编辑 | 基于已保存事实与人工反馈的周复盘和选题提示 | Implemented | M7 只读四类输入，输出发布/未发布/待核查、重复/更新提示和最多五条可追溯建议；不自动发布 |
...
| OPS-05 | 运维 | 运行诊断、非破坏性保留计划和隔离恢复验证 | Implemented | M5 离线诊断、候选计划、白名单复制和 Schema/引用/哈希检查；真实恢复演练后才能 Verified |
